...
def options(opt):
    ...
    opt.load(["default-compiler-flags", "boost", "ns3"], tooldir=[".waf-tools"])
    ...
    opt.add_option(
        ...
        default=True,
        dest="logging",
        help="""enable logging in simulation scripts""",
    )
    opt.add_option(
        "--run",
        help=(
            "Run a locally built program; argument can be a program name,"
    ...
    opt.add_option(
        "--mpi", help=("Run in MPI mode"), type="string", default="", dest="mpi"
    )
    opt.add_option(
        "--time",
...
def configure(conf):
    conf.load(["compiler_c", "compiler_cxx", "default-compiler-flags", "boost", "ns3"])

    if "PKG_CONFIG_PATH" not in os.environ.keys():
...
def build(bld):
    deps = " ".join(
        ["ns3_" + dep for dep in MANDATORY_NS3_MODULES + OTHER_NS3_MODULES]
    ).upper()
    ...
    for scenario in bld.path.ant_glob(["scenarios/*.cc", "scenarios/*.cpp"]):
        name = scenario.change_ext("").path_from(
            bld.path.find_node("scenarios/").get_bld()
        )
        app = bld.program(
            target=name,
            features=["cxx"],
...
def shutdown(ctx):
    if Options.options.run:
        ...
        if mpi and visualize:
            Logs.error(
                "You cannot specify --mpi and --visualize options at the same time!!!"
            )
            return
